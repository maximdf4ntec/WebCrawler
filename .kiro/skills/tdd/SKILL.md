...
name: tdd
description: >
  Test-driven development with a red-green-refactor loop for Python/pytest. Builds features or fixes bugs one vertical slice at a time. Activates when the user asks to implement a feature, fix a bug, or says "let's TDD this".
---
...
## Before starting
Read `GLOSSARY.md` / `CONTEXT.md` if present (see the `domain-modeling` skill) so test names and fixture vocabulary match the project's actual terms — "trajectory", "hard rule", "soft rule", "span", not ad-hoc synonyms.
