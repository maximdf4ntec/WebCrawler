...
description: Enforces the one-class-per-variant pattern for all domain concepts in this project. Activate when adding or reviewing any class that inherits from Rule, Parser, Exporter, or any other abstract domain base. Prevents procedural "bag-of-functions" antipattern.
---

# Pydantic Domain-Variants Skill

## Problem this prevents
A domain base class exists (`HardRule`, `SoftRule`, `Parser`, …) with typed fields and an
abstract method (`generate()`, `parse()`, …).  Instead of subclassing it, a developer writes
a collection of standalone functions (`_rule_a_fn(traj, cfg)`, `_rule_b_fn(traj, cfg)`, …)
that manually build and return base-class instances.  This is the **procedural bag-of-functions**
...
1. **Extends the correct base** (`HardRule`, `SoftRule`, `Parser`, etc.)
2. **Declares typed fields** for all variant-specific data (thresholds, observed values, param names…).
   No opaque `dict` or `Any` fields for known data.
3. **Overrides the abstract method** (`generate()`, `parse()`, …) to derive the output from its
   own typed fields — not from a pre-computed string passed at construction time.
4. **Uses a `model_validator(mode='after')`** (Pydantic v2) to derive string-serialisation fields
   (`pseudocode`, `rule_description`, …) from typed fields immediately after construction, so
   the instance is always self-consistent.
5. **Provides a factory classmethod** (`from_trajectory`, `from_step`, `from_file`, …) as the
...
- [ ] Class-level field defaults provided for all base-class fields that are constant per variant
      (e.g. `polarity: RulePolarity = RulePolarity.neutral`).
- [ ] No mutable default arguments (`field: list = []` — use `Field(default_factory=list)`).
- [ ] `from_trajectory` / `from_step` are `@classmethod`, not module-level functions.
...
## Violations
1. `hard_rules.py:_rule_a_redundancy` — standalone function builds `HardRule` directly.
   Fix: create `RedundancyRateRule(HardRule)` with `from_trajectory` classmethod.

## Compliant patterns found
- `ToolIndexRule.from_step` correctly encapsulates step-level logic.
```
